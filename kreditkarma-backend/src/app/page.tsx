...
// ─────────────────────────────────────────────────────────────────────────────
// THE 5 PRODUCTS  — each maps to a real XRPL transaction type
// ─────────────────────────────────────────────────────────────────────────────
const PRODUCTS = [
  {
    id: 'clawback',
  ...
  },
  {
    ...
    isMonthly: true,
    tiers: [
      { name: 'Starter',  priceRLUSD: 15,  priceXRP: 50,  color: '#34d399', perks: 'LedgerScore monitoring · Equifax soft reporting · Email alerts' },
      { name: 'Builder',  priceRLUSD: 25,  priceXRP: 80,  color: '#10b981', perks: 'All 3 bureaus soft · Score simulator · Dispute assist' },
      { name: 'Pro',      priceRLUSD: 35,  priceXRP: 115, color: '#f59e0b', perks: 'Hard bureau reporting · RLUSD rewards · DeFi pre-approval' },
    ],
...
function Overlay({ show, onClose, children, wide = false }: {
  ...
  if (!show) return null;
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,.9)', backdropFilter: 'blur(14px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ ...GLASS, borderRadius: 26, padding: '36px 34px', width: '100%', maxWidth: wide ? 700 : 520, position: 'relative', animation: 'popIn .26s cubic-bezier(.34,1.56,.64,1) both', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 0 80px rgba(16,185,129,.08),0 40px 100px rgba(0,0,0,.85)' }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// PRODUCT PURCHASE MODAL  — the heart of the UX
// ─────────────────────────────────────────────────────────────────────────────
function ProductModal({ show, onClose, product }: { show: boolean; onClose: () => void; product: Product | null }) {
  ...
  const [selectedTier, setSelectedTier] = useState(0); // for Credit Builder
  ...
  const handleClose = () => {
    onClose();
    setTimeout(() => { setStep('info'); setEmail(''); setCopied(false); setQrErr(false); setSelectedTier(0); }, 300);
  };
    ...
    <Overlay show={show} onClose={handleClose}>
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(16,185,129,.1)', border: '1px solid rgba(16,185,129,.25)', borderRadius: 99, padding: '4px 14px', marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: '#10b981', letterSpacing: '.1em' }}>SERVICE ACTIVE — XRPL MAINNET</span>
        </div>
  ...
  if (step === 'checkout') return (
    <Overlay show={show} onClose={handleClose}>
      ...
      {/* Tier selector for Credit Builder */}
      ...
      {/* Address */}
      <div style={{ background: 'rgba(16,185,129,.06)', border: '1px solid rgba(16,185,129,.18)', borderRadius: 12, padding: '10px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
        <code style={{ fontSize: 11, color: '#34d399', flex: 1, wordBreak: 'break-all', lineHeight: 1.5, fontFamily: "'IBM Plex Mono',monospace" }}>{TREASURY}</code>
  ...
  return (
    <Overlay show={show} onClose={handleClose} wide>
      ...
      <div style={{ background: 'rgba(16,185,129,.05)', border: '1px solid rgba(16,185,129,.18)', borderRadius: 14, padding: 16, marginBottom: 22 }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: '#10b981', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.09em', fontFamily: "'IBM Plex Mono',monospace" }}>🤖 What AI does after payment</p>
      ...
      {/* Pricing + CTA */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: `${product.color}08`, border: `1px solid ${product.color}22`, borderRadius: 14, padding: '18px 22px', marginBottom: 20 }}>
        <div>
    ...
    <Overlay show={show} onClose={onClose}>
      ...
      {scoreData&&!loading&&grade&&(
        <>
          ...
          {walletAddress&&<p style={{ fontSize:11,color:'rgba(255,255,255,.28)',fontFamily:"'IBM Plex Mono',monospace",textAlign:'center',marginBottom:16 }}>🔒 {walletAddress.slice(0,12)}…{walletAddress.slice(-6)}</p>}
  ...
  return (
    <Overlay show={show} onClose={onClose} wide>
      ...
      <div style={{ fontSize:14,color:'rgba(255,255,255,.65)',lineHeight:1.9,display:'flex',flexDirection:'column',gap:16 }}>
        ...
        <p><strong style={{ color:'#10b981' }}>LedgerScore</strong> is our flagship — the world's first on-chain credit standard. 300–850, built purely from wallet activity. No SSN. No bureau access required to start. Your Credit Builder subscription grows your score month by month, and our Pro tier feeds real data into Equifax, TransUnion, and Experian — the first-ever path from blockchain history to a traditional FICO score.</p>
        <p>Our <strong style={{ color:'#fff' }}>XRPL Amendment Services</strong> are AI-wrapped on-chain operational tools: real transactions (asfNoFreeze, EscrowCreate, TrustSet, AMMWithdraw) that AI agents file automatically to protect your positions. You pay once. AI watches forever.</p>
        <p>The <strong style={{ color:'#10b981' }}>Grant System</strong> is the most important part. No NGOs. No middlemen. No election funneling. Donors send RLUSD/XRP to the treasury — publicly visible at xrpscan.com. AI reviews applications. Approved funds go wallet-to-wallet in under 5 seconds. Permanently on-chain. Verifiable by anyone. Forever.</p>
        <p>We distribute physical business cards at Salvation Army locations, public libraries, and shelters. Each card has the website URL. A person in need can create a Xaman wallet in 60 seconds and apply for a grant without an ID, a bank account, or a credit history. That's the point.</p>
        <div style={{ background:'rgba(16,185,129,.06)',border:'1px solid rgba(16,185,129,.15)',borderRadius:14,padding:18 }}>
          <p style={{ fontSize:13,color:'rgba(255,255,255,.6)',lineHeight:1.75,fontStyle:'italic' }}>"We're not disrupting finance. We're replacing the parts that were never worth keeping — with math you can verify yourself."</p>
...
function FAQModal({ show, onClose }: { show:boolean; onClose:()=>void }) {
  ...
  const faqs: [string,string][] = [
    ...
    ['What is Credit Builder?', 'Three monthly subscription tiers (Starter/Builder/Pro) paid in RLUSD or XRP. Each payment builds your LedgerScore. Pro tier furnishes payment history to Equifax, TransUnion, and Experian — the first blockchain-to-FICO pathway ever built. You can have a FICO score without ever having had a bank account.'],
    ...
    ['How do I get started?', 'Get the Xaman wallet app (free, xaman.app, 60 seconds). Check your LedgerScore for free. Buy an Amendment Service, start a Credit Builder subscription, donate to the grant pool, or apply for emergency funds. All payments are made by scanning a QR code in Xaman and swiping to send.'],
    ['What about the Evernorth / XRPL Foundation partnership?', 'KreditKarma is actively pursuing institutional partnerships with Evernorth (health + financial wellness), the XRPL Foundation, and Ripple\'s community investment programs. If your organization manages a treasury or wants to fund health equity through blockchain grants, contact us at partners@kreditkarma.us.'],
  ];
  return (
    <Overlay show={show} onClose={onClose} wide>
...
export default function KreditKarmaHome() {
  ...
  return (
    <>
      ...
      {/* ── FIXED BACKGROUND (guarantees image always shows) ── */}
      <div style={{ position:'fixed',inset:0,zIndex:-1 }}>
        <div style={{ position:'absolute',inset:0,backgroundImage:"url('/xrpl-background.jpg')",backgroundSize:'cover',backgroundPosition:'center' }} />
        <div style={{ position:'absolute',inset:0,background:'linear-gradient(to bottom,rgba(3,3,10,.80) 0%,rgba(3,3,10,.68) 25%,rgba(3,3,10,.76) 65%,rgba(3,3,10,.96) 100%)' }} />
      </div>

      <div style={{ minHeight:'100vh',fontFamily:"'Syne',sans-serif",color:'#eeeef5' }}>

        {/* ── NAV ── */}
        <nav style={{ position:'sticky',top:0,zIndex:100,backdropFilter:'blur(22px)',WebkitBackdropFilter:'blur(22px)',background:"linear-gradient(rgba(3,3,10,.72),rgba(3,3,10,.82)), url('/xrpl-background.jpg') center/cover no-repeat",borderBottom:'1px solid rgba(16,185,129,.18)',padding:'0 28px',height:68,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12 }}>
          <div style={{ display:'flex',alignItems:'center',gap:9 }}>
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:1 }}>
              ...
              <span style={{ fontSize:9,color:'rgba(255,255,255,.45)',letterSpacing:'.07em',textTransform:'uppercase',lineHeight:1 }}>On-Chain Credit&nbsp;•&nbsp;XRPL Services&nbsp;•&nbsp;Real Grants</span>
            </div>
            <span style={{ display:'inline-flex',alignItems:'center',gap:5,background:'rgba(16,185,129,.1)',border:'1px solid rgba(16,185,129,.22)',borderRadius:99,padding:'3px 9px',fontSize:9,fontWeight:700,color:'#10b981',letterSpacing:'.1em',fontFamily:"'IBM Plex Mono',monospace" }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:7,flexWrap:'wrap' }}>
            ...
            {walletConnected&&(
              <div style={{ display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:99,border:'1px solid rgba(16,185,129,.35)',background:'rgba(16,185,129,.07)',fontSize:11,fontWeight:600,color:'#34d399',fontFamily:"'IBM Plex Mono',monospace" }}>
            ...
            <button onClick={()=>fetchScore()} style={{ padding:'8px 18px',borderRadius:99,fontFamily:'inherit',fontWeight:700,fontSize:13,cursor:'pointer',border:'none',background:'#10b981',color:'#000',transition:'all .15s' }}>Get LedgerScore</button>
          </div>
        </nav>

        {/* ── HERO ── */}
        <section style={{ textAlign:'center',padding:'100px 24px 72px',position:'relative',overflow:'hidden' }}>
          <div style={{ position:'absolute',top:'35%',left:'50%',transform:'translate(-50%,-50%)',width:700,height:700,borderRadius:'50%',background:'radial-gradient(circle,rgba(16,185,129,.07) 0%,transparent 68%)',pointerEvents:'none',animation:'float 9s ease-in-out infinite' }} />

          <div style={{ display:'inline-flex',alignItems:'center',gap:8,background:'rgba(16,185,129,.1)',border:'1px solid rgba(16,185,129,.28)',color:'#10b981',padding:'6px 16px',borderRadius:99,fontSize:10,fontWeight:700,marginBottom:32,letterSpacing:'.09em',fontFamily:"'IBM Plex Mono',monospace" }}>
            ...
            FIRST-TO-MARKET · XRPL BLOCKCHAIN CREDIT & AMENDMENT SERVICES · {new Date().getFullYear()}
          </div>

          <h1 style={{ fontSize:'clamp(46px,8.5vw,96px)',fontWeight:900,letterSpacing:'-4px',lineHeight:.91,marginBottom:22 }}>
            KreditKarma<br />
          ...
          <p style={{ fontSize:18,color:'rgba(255,255,255,.52)',maxWidth:540,margin:'0 auto 48px',lineHeight:1.7 }}>
          ...
          <div style={{ display:'flex',gap:14,justifyContent:'center',flexWrap:'wrap',marginBottom:44 }}>
            <button className="hero-p" onClick={()=>fetchScore()} style={{ display:'inline-flex',alignItems:'center',gap:8,padding:'17px 38px',background:'#fff',color:'#000',fontSize:17,fontWeight:700,borderRadius:99,border:'none',cursor:'pointer',transition:'all .18s',boxShadow:'0 4px 28px rgba(255,255,255,.12)' }}>
              Get My Free LedgerScore →
            </button>
            <button className="hero-g" onClick={()=>{ const el=document.getElementById('products'); el?.scrollIntoView({behavior:'smooth'}); }} style={{ display:'inline-flex',alignItems:'center',gap:8,padding:'17px 38px',border:'1.5px solid rgba(255,255,255,.22)',color:'#fff',fontSize:17,fontWeight:600,borderRadius:99,background:'transparent',cursor:'pointer',transition:'all .18s',backdropFilter:'blur(8px)' }}>
              Browse Services ↓
            </button>
          </div>

          <div style={{ display:'flex',gap:32,justifyContent:'center',flexWrap:'wrap' }}>
            {[['42,891','LedgerScores'],['$1.2M','Grants Funded'],['0%','Overhead'],['<5s','To Wallet'],['5','Amendment Services']].map(([n,l])=>(
              <div key={l} style={{ textAlign:'center' }}>
                <div style={{ fontSize:24,fontWeight:900,color:'#10b981',lineHeight:1,textShadow:'0 0 30px rgba(16,185,129,.5)' }}>{n}</div>
        ...
        {/* ── PRODUCTS — TOP 3 FEATURED ── */}
        <section id="products" style={{ padding:'0 28px 80px',maxWidth:1240,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:48 }}>
            ...
            <h2 style={{ fontSize:'clamp(26px,4vw,46px)',fontWeight:900,letterSpacing:'-2px',marginBottom:12 }}>AI-delivered on-chain protection</h2>
          ...
          {/* 3 featured big cards */}
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:20,marginBottom:20 }}>
            {featuredProducts.map(p=>(
              <div key={p.id} className="pcard-featured" onClick={()=>setActiveProduct(p)}
                style={{ background:`linear-gradient(135deg,${p.color}10,rgba(6,6,22,.8))`,border:`1px solid ${p.color}30`,borderRadius:24,padding:30,position:'relative',overflow:'hidden' }}>
                ...
                <div style={{ width:52,height:52,borderRadius:14,background:`${p.color}18`,border:`1px solid ${p.color}28`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,marginBottom:16,animation:'float 4s ease-in-out infinite' }}>{p.emoji}</div>
                ...
                <p style={{ fontSize:12,color:'rgba(255,255,255,.46)',lineHeight:1.7,marginBottom:18 }}>{p.tagline}</p>
                ...
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',borderTop:`1px solid ${p.color}18`,paddingTop:16 }}>
                  <div>
                    <span style={{ fontSize:22,fontWeight:900,color:p.color }}>{p.priceRLUSD} RLUSD</span>
                    <span style={{ fontSize:11,color:'rgba(255,255,255,.3)',marginLeft:8 }}>or {p.priceXRP} XRP</span>
                  </div>
                  <button onClick={()=>setActiveProduct(p)} style={{ padding:'9px 18px',borderRadius:99,background:p.color,color:'#000',border:'none',fontWeight:800,fontSize:12,cursor:'pointer',fontFamily:'inherit',transition:'all .15s' }}>
                    Buy Now →
                  </button>
                </div>
              </div>
          ...
          {/* 2 other products — smaller */}
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:16 }}>
            {otherProducts.map(p=>(
              <div key={p.id} className="pcard" onClick={()=>setActiveProduct(p)}
                style={{ background:'rgba(6,6,22,.72)',backdropFilter:'blur(16px)',border:`1px solid ${p.color}20`,borderRadius:18,padding:24 }}>
                <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:14 }}>
                  <div style={{ width:44,height:44,borderRadius:12,background:`${p.color}18`,border:`1px solid ${p.color}25`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20 }}>{p.emoji}</div>
                ...
                <p style={{ fontSize:12,color:'rgba(255,255,255,.42)',lineHeight:1.65,marginBottom:16 }}>{p.tagline}</p>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:14,borderTop:`1px solid ${p.color}15` }}>
                  <span style={{ fontSize:17,fontWeight:900,color:p.color }}>{p.priceRLUSD} RLUSD{p.id==='credit'?'/mo':''}</span>
        ...
        {/* ── LEDGERSCORE ── */}
        <section id="score" style={{ padding:'0 28px 80px',maxWidth:1240,margin:'0 auto' }}>
          <div style={{ background:'linear-gradient(135deg,rgba(16,185,129,.07),rgba(6,6,22,.8))',border:'1px solid rgba(16,185,129,.18)',borderRadius:26,padding:'52px 44px',backdropFilter:'blur(20px)',animation:'borderPulse 4s ease-in-out infinite' }}>
            <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(340px,1fr))',gap:48,alignItems:'center' }}>
              <div>
                ...
                <h2 style={{ fontSize:'clamp(26px,3.5vw,44px)',fontWeight:900,letterSpacing:'-2px',marginBottom:16 }}>
              ...
              <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:11 }}>
                  {[
                    { name:'Starter', price:'15 RLUSD/mo',   color:'#34d399', sub:'LedgerScore monitoring · Equifax soft · Email alerts' },
                    { name:'Builder', price:'25 RLUSD/mo',   color:'#10b981', sub:'All 3 bureaus soft · Score simulator · Dispute assist', popular:true },
                    { name:'Pro',     price:'35 RLUSD/mo',   color:'#f59e0b', sub:'Hard bureau reporting · RLUSD rewards · DeFi pre-approval' },
                  ].map(t=>(
        ...
        {/* ── GRANTS ── */}
        <section id="grants" style={{ padding:'0 28px 80px',maxWidth:1240,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:44 }}>
            ...
            <h2 style={{ fontSize:'clamp(26px,4vw,46px)',fontWeight:900,letterSpacing:'-2px',marginBottom:12 }}>Real people. Real money. Wallet to wallet.</h2>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(320px,1fr))',gap:22 }}>
            {/* Donate */}
            <div style={{ background:'linear-gradient(135deg,rgba(16,185,129,.08),rgba(6,6,22,.8))',border:'1px solid rgba(16,185,129,.2)',borderRadius:22,padding:'34px 30px',backdropFilter:'blur(20px)' }}>
              <div style={{ fontSize:42,marginBottom:16,animation:'float 4s ease-in-out infinite' }}>💚</div>
              <h3 style={{ fontSize:22,fontWeight:900,marginBottom:10 }}>Fund the Treasury</h3>
              <p style={{ fontSize:13,color:'rgba(255,255,255,.48)',lineHeight:1.8,marginBottom:24 }}>
                Send XRP or RLUSD. <strong style={{ color:'#fff' }}>100%</strong> goes to grant recipients. Treasury is public on XRPScan. Every transaction verifiable by anyone, forever. That's not a promise. Math.
              </p>
              ...
              <div style={{ background:'rgba(16,185,129,.06)',border:'1px solid rgba(16,185,129,.15)',borderRadius:11,padding:'9px 13px',marginBottom:20 }}>
            ...
            {/* Apply */}
            <div style={{ background:'linear-gradient(135deg,rgba(139,92,246,.08),rgba(6,6,22,.8))',border:'1px solid rgba(139,92,246,.2)',borderRadius:22,padding:'34px 30px',backdropFilter:'blur(20px)' }}>
              <div style={{ fontSize:42,marginBottom:16,animation:'float 4s ease-in-out infinite',animationDelay:'1s' }}>❤️</div>
              <h3 style={{ fontSize:22,fontWeight:900,marginBottom:10 }}>Apply for Emergency Aid</h3>
              <p style={{ fontSize:13,color:'rgba(255,255,255,.48)',lineHeight:1.8,marginBottom:24 }}>
                Need help? Apply for $25–$100. AI reviews every application within 24 hours — no bias, no gatekeepers. Approved funds go directly to your XRPL wallet. No bank account required. No paperwork.
              </p>
              <div style={{ display:'flex',flexDirection:'column',gap:8,marginBottom:24 }}>
                {['$25–$100 direct to your XRPL wallet','AI review within 24 hours — no bias','No bank account, no ID required','Permanently verifiable on XRPScan','Business cards distributed at shelters + libraries'].map(f=>(
        ...
        {/* ── WHY XRPL ── */}
        <section style={{ padding:'0 28px 80px',maxWidth:1240,margin:'0 auto' }}>
          <div style={{ background:'rgba(6,6,22,.72)',backdropFilter:'blur(20px)',border:'1px solid rgba(255,255,255,.08)',borderRadius:22,padding:'44px 40px',textAlign:'center' }}>
            <div style={{ display:'inline-flex',alignItems:'center',gap:6,marginBottom:14 }}>
              <span style={{ width:5,height:5,borderRadius:'50%',background:'#10b981',boxShadow:'0 0 8px #10b981',display:'inline-block' }} />
              <span style={{ fontSize:11,fontWeight:700,color:'#10b981',letterSpacing:'.14em',textTransform:'uppercase' }}>Why XRPL</span>
            </div>
            <h2 style={{ fontSize:'clamp(22px,3.5vw,38px)',fontWeight:900,letterSpacing:'-2px',marginBottom:14 }}>The only blockchain built for this.</h2>
            <p style={{ fontSize:13,color:'rgba(255,255,255,.44)',maxWidth:480,margin:'0 auto 36px',lineHeight:1.7 }}>
            ...
            <div style={{ display:'flex',justifyContent:'center',gap:32,flexWrap:'wrap' }}>
              {[['3–5s','Finality'],['$0.000001','Avg Fee'],['10+','Years Live'],['1,500','TPS'],['~0.0079kWh','Per TX']].map(([v,l])=>(
                <div key={l} style={{ textAlign:'center',minWidth:80 }}>
                  <div style={{ fontSize:26,fontWeight:900,color:'#10b981',textShadow:'0 0 28px rgba(16,185,129,.5)' }}>{v}</div>
                  <div style={{ fontSize:10,color:'rgba(255,255,255,.35)',marginTop:4,textTransform:'uppercase',letterSpacing:'.08em' }}>{l}</div>
                </div>
              ))}
            </div>
        ...
        <section style={{ padding:'0 28px 100px',maxWidth:820,margin:'0 auto',textAlign:'center' }}>
          <div style={{ background:'linear-gradient(135deg,rgba(16,185,129,.1),rgba(6,6,22,.85))',border:'1px solid rgba(16,185,129,.24)',borderRadius:26,padding:'60px 44px',backdropFilter:'blur(20px)' }}>
            <h2 style={{ fontSize:'clamp(26px,5vw,52px)',fontWeight:900,letterSpacing:'-2.5px',lineHeight:1.04,marginBottom:16 }}>
              Your wallet has a story.<br />
              <span style={{ color:'#10b981' }}>Let the ledger score it.</span>
            </h2>
            <p style={{ fontSize:15,color:'rgba(255,255,255,.44)',marginBottom:34,lineHeight:1.65 }}>Free. No SSN. No bank account. 100% on-chain. This is the XRPL build of the century.</p>
            <div style={{ display:'flex',gap:14,justifyContent:'center',flexWrap:'wrap' }}>
              <button className="hero-p" onClick={()=>fetchScore()} style={{ display:'inline-flex',alignItems:'center',gap:8,padding:'16px 36px',background:'#fff',color:'#000',fontSize:16,fontWeight:700,borderRadius:99,border:'none',cursor:'pointer',transition:'all .18s',boxShadow:'0 4px 28px rgba(255,255,255,.12)' }}>Get Free LedgerScore →</button>
              <button className="hero-g" onClick={()=>setShowDonate(true)} style={{ display:'inline-flex',alignItems:'center',gap:8,padding:'16px 36px',border:'1.5px solid rgba(255,255,255,.22)',color:'#fff',fontSize:16,fontWeight:600,borderRadius:99,background:'transparent',cursor:'pointer',transition:'all .18s' }}>💚 Fund a Grant</button>
            </div>
            <p style={{ fontSize:11,color:'rgba(255,255,255,.22)',marginTop:20 }}>
              Seeking institutional partnerships · Evernorth · XRPL Foundation · Ripple · contact: <a href="mailto:partners@kreditkarma.us" style={{ color:'#10b981' }}>partners@kreditkarma.us</a>
            </p>
          </div>
        </section>

        {/* ── FOOTER ── */}
        <footer style={{ backdropFilter:'blur(14px)',background:'rgba(3,3,10,.76)',borderTop:'1px solid rgba(255,255,255,.07)',padding:'36px 28px 28px' }}>
